...
describe Object do
  describe '.biggest' do
    context 'element nil' do
      it 'return zero' do
        expect(subject.biggest).to eq(0)
      end
    end

    context 'element void' do
      it 'retorna cero' do
        expect(subject.biggest('')).to eq('')
      end
    end
    context 'one element' do
      it 'return element' do
        expect(subject.biggest('word')).to eq('word')
      end
    end

    context 'elements with the same length' do
      it 'return the first element' do
        expect(subject.biggest('one', 'two')).to eq('one')
      end
    end

    context 'numbers' do
      it 'return Invalid' do
        expect(subject.biggest(1)).to eq('Invalid')
      end
    end
  end
end
